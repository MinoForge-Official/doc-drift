...
import { DocDriftOptions, DocDriftReport, BlockTestResult } from './types.js';
...
function findMarkdownFiles(targets: string[], cwd: string): string[] {
  const files: string[] = [];
...
export function runDocDrift(options: DocDriftOptions = {}): DocDriftReport {
  ...
  for (const file of mdFiles) {
    ...
    for (const block of blocks) {
      ...
      if (res.status === 'pass') passedBlocks++;
      else if (res.status === 'fail') failedBlocks++;
      else if (res.status === 'skipped') skippedBlocks++;
    }
  }
